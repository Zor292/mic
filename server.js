...
try {
  const configuredValue = String(process.env.TURN_ICE_SERVERS || "")
    .replace(/^TURN_ICE_SERVERS\s*=\s*/, "")
    .trim();

  const configuredIceServers = JSON.parse(configuredValue || "[]");

  if (Array.isArray(configuredIceServers)) {
    iceServers.push(...configuredIceServers);
  }
} catch {
  console.warn("TURN_ICE_SERVERS is not valid JSON");
}
...
function broadcast(room) {
  for (const client of room.clients) {
    ...
    for (const other of room.clients) {
      ...
      const sameRadio = Boolean(
        me.radioChannel &&
        player.radioChannel &&
        me.radioChannel === player.radioChannel
      );

      if (meters <= connectionRange || sameRadio) {
        peers.push({
          id: other.socketId,
          username: player.displayName,
          distance: Math.round(meters * 10) / 10,
          position: player.position,
          proximity: meters <= proximityRange,
          muted: Boolean(other.muted || player.muted),
          radio: sameRadio,
          radioOnly: sameRadio && meters > proximityRange,
          radioTalking: Boolean(player.radioTalking),
          radioChannel: sameRadio ? me.radioChannel : null
        });
      }
    }
    ...
    client.send(JSON.stringify({
      ...
      self: {
        ...
        muted: Boolean(client.muted || me.muted),
        radioChannel: me.radioChannel || null,
        radioTalking: Boolean(me.radioTalking)
      }
    }));
  }
}
...
function broadcastRadioBeep(room, channel, speakerId) {
  for (const client of room.clients) {
    ...
    if (player?.radioChannel === channel) {
      client.send(JSON.stringify({
        type: "radioBeep"
      }));
    }
...
app.post("/api/roblox/register", robloxAuth, (req, res) => {
  ...
  room.players.set(userId, {
    ...
    radioChannel: text(req.body.radioChannel) || null,
    radioTalking: false,
    lastSeen: Date.now()
  });
  ...
  for (const item of players) {
    ...
    if (!room.players.has(userId)) {
      room.players.set(userId, {
        ...
        radioChannel: text(item.radioChannel) || null,
        radioTalking: Boolean(item.radioTalking),
        lastSeen: Date.now()
      });
    }
    ...
    player.radioTalking = Boolean(
      item.radioTalking &&
      player.radioChannel
    );
    player.lastSeen = Date.now();

    if (!wasTalking && player.radioTalking) {
      broadcastRadioBeep(
        room,
        player.radioChannel,
        userId
...
app.post("/api/roblox/radio-talk", robloxAuth, (req, res) => {
  ...
  if (
    ...
    return res.status(403).json({
      error: "Radio channel is not active"
    });
  }
  ...
  if (!wasTalking && talking) {
    broadcastRadioBeep(room, channel, userId);
  }
...
server.on("upgrade", (request, socket, head) => {
  ...
  if (
    !identity ||
    Date.now() - identity.createdAt > 86400000
  ) {
    return socket.destroy();
  }
  ...
  ws.on("message", raw => {
    ...
    if (
      ...
      const target = [...room.clients].find(
        client => client.socketId === message.targetId
      );
    ...
    if (message.type === "mute") {
      ws.muted = Boolean(message.muted);
      broadcast(room);
    }
  });
...
setInterval(() => {
  for (const [roomId, room] of rooms) {
    for (const [userId, player] of room.players) {
      if (Date.now() - player.lastSeen > 15000) {
        closePlayerClients(room, userId);
        room.players.delete(userId);
      }
    }
...
server.listen(port, () => {
  console.log(`VoiceLink listening on ${publicUrl}`);
});
