...
function createPeer(peer, initiator) {
  ...
  pc.oniceconnectionstatechange = () => { if (pc.iceConnectionState === "failed") { $("connectionText").textContent = "فشل اتصال الصوت"; } if (["connected", "completed"].includes(pc.iceConnectionState)) { $("connectionText").textContent = "متصل"; } };
  pc.onconnectionstatechange = () => { if (["failed", "closed"].includes(pc.connectionState)) { pc.close(); state.connections.delete(peer.id); } };
